...
export async function POST(
  ...
  const body =
    (await req.json()) as SubmitAchievementRequest
  ...
  if (
    ...
    return Response.json(
      {
        error:
          "Missing required fields: period, cycle_id, actual_achievement, progress_status",
      },
      { status: 400 }
    )
  }
  ...
  if (cycle) {
    ...
    if (
      ...
      return Response.json(
        {
          error: `The ${period} check-in window opens on ${new Date(
            openDate
          ).toLocaleDateString()}.`,
        },
        { status: 422 }
